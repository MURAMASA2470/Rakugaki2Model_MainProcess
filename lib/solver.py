...
class Solver(object):
    ...
    def train(self, train_queue, val_queue=None):
        ...
        for train_ind in range(start_iter, cfg.TRAIN.NUM_ITERATION + 1):
            ...
            train_timer.tic()
            # print(f'batch_img: {batch_img}')
            # loss = self.train_loss(batch_img, batch_voxel)
            loss = self.train_loss(batch_img, batch_voxel)
            print('なんで届いてないんや')
            print(f'loss: {loss}') # 追加
            train_timer.toc()
            ...
            print(f'セーブの可否: {(train_ind % cfg.TRAIN.SAVE_FREQ == 0 and not train_ind == 0)}')
